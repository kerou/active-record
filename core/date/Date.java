...
public class Date extends java.util.Date{
    ...
    public String format(String fmt, String date_fmt){
        ...
        if(isToday())      date_string = Application.getContext().getString(Res.getId(Application.getContext().getPackageName(), "string", "today"));
        if(isYesterday())  date_string = Application.getContext().getString(Res.getId(Application.getContext().getPackageName(), "string", "yesterday"));
        if(isTomorrow())   date_string = Application.getContext().getString(Res.getId(Application.getContext().getPackageName(), "string", "tomorrow"));
    ...
    static public boolean isYesterday(java.util.Date date){
    ...
    static public boolean isTomorrow(java.util.Date date){
        java.util.Date test = new java.util.Date(System.currentTimeMillis() + (1000*60*60*24));
        return  date.getDate () == test.getDate () &&
                date.getMonth() == test.getMonth() &&
                date.getYear() == test.getYear();
